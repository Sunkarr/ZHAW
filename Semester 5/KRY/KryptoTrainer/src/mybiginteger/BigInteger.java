...
public class BigInteger
  ...
  public BigInteger myModPow(BigInteger exponent, BigInteger m) {
       * Create a for loop that computes (this^exponent) mod (p*q) using the Chinese Remainder Theorem.
       * p and q are distinct primes. The method should be faster than myModPow(exponent, p*q).
       * You may use myModPow(exponent, p) and myModPow(exponent, q) in your implementation.
       * Hint: Use modInverse() from the BigInteger class to compute the inverse of q mod p
       * and the inverse of p mod q.


      BigInteger result = BigInteger.ONE;
      for (int i = 0; i < exponent.intValue(); i++) {
          result = result.multiply(this).mod(m);
      }
      return result;
  ...
  */
  ...
  /*Speichern Sie eine Kopie Ihrer Methode myModPow() aus Aufgabe (3) als Kommentar.
  Verbessern Sie sodann myModPow(), indem𝑏𝑏 Sie den Trick des fortgesetzten Quadrierens anwenden:
  Es soll wiederum der Ausdruck 𝑎𝑎 ( 𝑚𝑚𝑚𝑚𝑚𝑚 𝑚𝑚 ) berechnet werden. Im Binärsystem lässt sich der Exponent als
  Summe 3 von aufsteigenden Zweierpotenzen darstellen:
  𝑏𝑏 = 2 𝑘𝑘 1 + 2 𝑘𝑘 2 + 2 𝑘𝑘 + ⋯ + 2 𝑘𝑘 𝑟𝑟 , mit 𝑘𝑘 1 < 𝑘𝑘 2 < 𝑘𝑘 3 < ⋯ < 𝑘𝑘 𝑟𝑟 .
  Damit lässt sich der Ausdruck 𝑎𝑎 𝑏𝑏 ( 𝑚𝑚𝑚𝑚𝑚𝑚 𝑚𝑚 ) mit Hilfe der Potenzgesetze berechnen nach der Formel

*/

  public BigInteger myModPow(BigInteger exponent, BigInteger m) {
    BigInteger result = BigInteger.ONE;
    BigInteger base = this.mod(m);
    ...
    while (exp.compareTo(BigInteger.ZERO) > 0) {
      ...
      exp = exp.shiftRight(1); // Divide exp by 2
      base = base.multiply(base).mod(m); // Square the base
    }
    return result;
  }
  ...
  /**
   * Returns the square root of this mod p, if it exists and BigInteger -1 otherwise.
  ...
  /**
   * Returns a positive BigInteger that is probably a safe prime, with the
